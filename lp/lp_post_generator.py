...
def generate_news_hook(article: dict) -> dict:
    ...
    user_msg = (
        ...
        f"1. Translates any technical or financial jargon into plain everyday language\n"
        f"   - DON'T say 'BSP pausing rate cuts' — DO say 'borrowing costs staying high' or 'loans not getting cheaper'\n"
        f"   - DON'T say 'monetary policy' — DO say 'how the government controls money'\n"
        f"   - DON'T say 'inflation rate' — DO say 'prices going up'\n"
        f"   - DON'T say 'peso depreciation' — DO say 'our money buys less'\n"
        f"   - Always translate the impact into: grocery bills, fuel, rent, daily expenses\n"
        f"2. Sound like a real person talking to a friend — not a news anchor, not AI\n"
        f"   - Short sentences. Direct. Casual but thoughtful.\n"
        f"   - Use 'we' throughout — couple speaking together\n"
        f"   - No formal transitions like 'it makes us think about how...'\n"
        f"   - Write how you'd say it out loud, not how you'd write an essay\n"
        f"3. Make the audience feel seen — they are already feeling the impact in their wallet\n"
        f"4. End with a SHORT genuine question (max 10 words) that invites them to share\n"
        f"5. NO product/business/opportunity mentions\n"
        f"6. Keep the whole post under 60 words\n\n"
        f"Output format (use exactly):\n"
        f"POST: [post body]\n"
        f"CAPTION: [2–6 words — conversational reaction, NOT a news headline. "
        f"Sound like a person. Optional Taglish. Examples: 'Nararamdaman na natin.', "
        f"'This one hits close.', 'Totoo 'to.', 'Ganito talaga ngayon.']"
    )
